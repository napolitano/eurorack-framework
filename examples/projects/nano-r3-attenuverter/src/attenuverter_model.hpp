 ...
 */
[[nodiscard]] inline float applyCenteredDeadZone(
    const float value,
    const float halfWidth) noexcept {

    const float clampedValue = std::clamp(value, -1.0F, 1.0F);
    const float clampedWidth = std::clamp(halfWidth, 0.0F, 0.999F);
    ...
    const float rescaled =
        (magnitude - clampedWidth) / (1.0F - clampedWidth);
 ...
 */
inline void calculateSideScales(
    const float balance,
    float& positiveScale,
    float& negativeScale) noexcept {
 ...
 */
[[nodiscard]] inline AttenuverterResult process(
    const float inputVolts,
    const float attenuation,
    const float balance) noexcept {

    AttenuverterResult result{};
    result.inputVolts = inputVolts;
    result.attenuation = std::clamp(attenuation, -1.0F, 1.0F);
    result.balance = std::clamp(balance, -1.0F, 1.0F);

    calculateSideScales(
        result.balance,
        result.positiveScale,
        result.negativeScale);

    result.attenuatedVolts =
        result.inputVolts * result.attenuation;

    result.outputVolts =
        result.attenuatedVolts >= 0.0F
