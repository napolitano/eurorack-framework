...
void lifecycle_and_page_addressing() {
    ...
    TEST_ASSERT_EQUAL_HEX8(0xB0U, bus.transfers()[before].written[1]);
    TEST_ASSERT_EQUAL_HEX8(0x02U, bus.transfers()[before].written[2]);
}
...
void invalid_geometry_is_rejected() {
    ...
    drivers::display::Sh1106Config cfg;
    cfg.width = 129U;
    drivers::display::Sh1106 wide(bus, cfg);
    TEST_ASSERT_EQUAL_INT(0, (int)wide.initialize());
    std::array<std::uint8_t, 2048> big{};
    display::MonochromeCanvas bc(big.data(), big.size(), 129, 64);
    TEST_ASSERT_EQUAL_INT((int)io::IoResult::InvalidArgument, (int)wide.flush(bc));
}
void errors_propagate() {
    simulation::VirtualI2cBus bus;
    drivers::display::Sh1106 d(bus);
    ...
    drivers::display::Sh1106 d2(bus2);
...
int main() {
    UNITY_BEGIN();
    RUN_TEST(lifecycle_and_page_addressing);
    RUN_TEST(controls_are_encoded);
    RUN_TEST(invalid_geometry_is_rejected);
    RUN_TEST(errors_propagate);
    return UNITY_END();
}
