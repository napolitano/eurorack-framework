 ...
 * Neither contact asserted represents center Off. The maintained contact selects On, and the
 * spring-return contact selects MomentaryOn. Simultaneous assertion is preserved as Invalid rather
 * than resolved by priority, making wiring or sampling faults visible to application logic.
 *
...
OnOffMomentarySwitch::OnOffMomentarySwitch(const OnOffMomentarySwitchConfig config) noexcept
    : config_(config), maintained_({config.maintainedActiveLevel, config.debounceTimeMs}),
      momentary_({config.momentaryActiveLevel, config.debounceTimeMs}) {}

void OnOffMomentarySwitch::reset(const bool maintainedRawHigh,
...
void OnOffMomentarySwitch::update(const bool maintainedRawHigh,
                                  const bool momentaryRawHigh,
                                  const std::uint32_t nowMs) noexcept {
