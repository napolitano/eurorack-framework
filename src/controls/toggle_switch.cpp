 ...
 *
 * @details
 * Uses the existing momentary-button debounce state machine while exposing maintained-switch
 * terminology.
 *
...
ToggleSwitch::ToggleSwitch(const ToggleSwitchConfig config) noexcept
    : config_(config), debouncer_({config.activeLevel, config.debounceTimeMs}) {}

void ToggleSwitch::reset(const bool rawLevelHigh, const std::uint32_t nowMs) noexcept {
    debouncer_.reset(rawLevelHigh, nowMs);
    snapshot_ = {};
    snapshot_.position =
        debouncer_.isPressed() ? ToggleSwitchPosition::On : ToggleSwitchPosition::Off;
}
...
const ToggleSwitchSnapshot& ToggleSwitch::snapshot() const noexcept {
    return snapshot_;
}
bool ToggleSwitch::isOn() const noexcept {
    return snapshot_.position == ToggleSwitchPosition::On;
}
const ToggleSwitchConfig& ToggleSwitch::config() const noexcept {
    return config_;
}
