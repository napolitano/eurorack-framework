 ...
 *
 * @details
 * Represents maintained binary configuration switches and provides allocation-free compile-time-sized banks with bit-mask conversion.
 *
...
public:
    ...
    /** @brief Resets the pole. @param rawLevelHigh Electrical input level. @param nowMs Monotonic milliseconds. */
    void reset(bool rawLevelHigh, std::uint32_t nowMs) noexcept;

    /** @brief Processes one sample. @param rawLevelHigh Electrical input level. @param nowMs Monotonic milliseconds. */
...
class DipSwitchBank final {
    static_assert(Count > 0U && Count <= 32U,
                  "DipSwitchBank supports between 1 and 32 poles");

public:
    /** @brief Constructs every pole with identical settings. @param config Shared pole configuration. */
     ...
     */
    void reset(const std::array<bool, Count>& rawLevelsHigh,
     ...
     * @param nowMs Monotonic milliseconds.
     */
    void update(const std::array<bool, Count>& rawLevelsHigh,
                const std::uint32_t nowMs) noexcept {
        for (std::size_t index = 0U; index < Count; ++index) {
    ...
    /** @brief Returns one pole. @param index Zero-based pole index. @return Constant pole reference. */
