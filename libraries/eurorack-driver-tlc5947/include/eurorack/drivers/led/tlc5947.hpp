...
class Tlc5947 final : public LedBank {
  public:
    ...
    /** @brief Reports whether the supplied buffers and startup dependencies are valid. @return True for a usable driver. */
    [[nodiscard]] bool valid() const noexcept;
    /** @brief Initializes the driver and performs the configured startup sequence. @return Success or the first validation/SPI error. */
    eurorack::io::IoResult initialize() noexcept;
    /** @brief Returns the number of logical channels. @return Twenty-four channels per configured device, or zero when invalid. */
    [[nodiscard]] std::size_t channelCount() const noexcept override;
    /** @brief Stores one logical brightness value. @param channel Zero-based logical channel. @param brightness Sixteen-bit logical brightness. @return Success or InvalidArgument. */
    eurorack::io::IoResult setBrightness(std::size_t channel, std::uint16_t brightness) noexcept override;
    /** @brief Reads one buffered logical brightness. @param channel Zero-based logical channel. @return Buffered value, or zero for an invalid channel. */
    [[nodiscard]] std::uint16_t brightness(std::size_t channel) const noexcept override;
    /** @brief Clears all caller-owned logical brightness entries. @return Success or InvalidArgument. */
    eurorack::io::IoResult clear() noexcept override;
    /** @brief Packs, transfers, and latches one complete frame. @return Success or the first SPI/validation error. */
    eurorack::io::IoResult flush() noexcept override;
    /** @brief Controls the optional active-low BLANK/OE pin. @param enabled True to make latched PWM data visible. */
    void setEnabled(bool enabled) noexcept;

  private:
    /** @brief Packs all logical brightness values into the caller-owned wire-format buffer. */
    void buildFrame() noexcept;
    /** @brief Executes the configured diagnostic startup policy. @return Success or the first SPI error. */
    eurorack::io::IoResult runStartupSequence() noexcept;
    /** @brief Converts milliseconds to the delay-provider unit. @param milliseconds Requested blocking delay. */
    void delayMilliseconds(std::uint16_t milliseconds) noexcept;

    eurorack::io::SpiBus& spi_;
    eurorack::io::DigitalOutput& latch_;
    eurorack::io::DigitalOutput* oe_;
    eurorack::io::DelayProvider* delay_;
    std::uint16_t* values_;
    std::uint8_t* frame_;
    std::size_t channels_;
    std::size_t frameSize_;
    Tlc5947Config config_{};
    bool valid_{false};
