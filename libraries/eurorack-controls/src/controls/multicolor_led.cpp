 ...
 *
 * Each effective channel is calculated with a 32-bit intermediate and rounded back to 16 bits.
 * Transition metadata changes only when effective optical intent changes, not merely when a setter
...
namespace {
[[nodiscard]] std::uint16_t scaleChannel(const std::uint16_t channel,
                                         const std::uint16_t master) noexcept {
    const std::uint32_t product =
        static_cast<std::uint32_t>(channel) * static_cast<std::uint32_t>(master);
    return static_cast<std::uint16_t>((product + 32767U) / 65535U);
}
...
MulticolorLed::MulticolorLed(const RgbColor initialColor,
                             const std::uint16_t masterBrightness) noexcept
    : initialColor_(initialColor), initialMasterBrightness_(masterBrightness) {
    reset();
}
...
void MulticolorLed::reset() noexcept {
    ...
    snapshot_.effective = {scaleChannel(initialColor_.red, initialMasterBrightness_),
...
void MulticolorLed::applyTo(eurorack::drivers::led::LedChannel& red,
...
const MulticolorLedSnapshot& MulticolorLed::snapshot() const noexcept {
    return snapshot_;
}

void MulticolorLed::recalculate() noexcept {
    const RgbColor previous = snapshot_.effective;
    snapshot_.effective = {scaleChannel(snapshot_.requested.red, snapshot_.masterBrightness),
